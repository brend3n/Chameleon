# #!/bin/bash

# # Configuration file to store MAC address history
# CONFIG_FILE="$HOME/.macchanger_history"

# # File to store the original MAC address in write-only mode
# OG_MAC_FILE="$HOME/.original_mac_write_only"

# # Function to display the current MAC address
# display_current_mac() {
#     current_mac=$(ifconfig $interface | awk '/ether/ {print $2}')
#     echo "Current MAC address for $interface: $current_mac"
# }

# # Function to change the MAC address to a specific value
# change_to_specific_mac() {
#     # Read user input for the new MAC address
#     read -p "Enter the new MAC address: " new_mac

#     # Validate the entered MAC address format
#     if [[ ! $new_mac =~ ^([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}$ ]]; then
#         echo "Invalid MAC address format. Please enter a valid address."
#         return
#     fi

#     # Disable the network interface
#     ifconfig $interface down

#     # Change the MAC address
#     ifconfig $interface hw ether $new_mac

#     # Enable the network interface
#     ifconfig $interface up

#     # Display the new MAC address
#     echo "Changed MAC address of $interface to $new_mac"

#     # Save the new MAC address to the history file
#     echo "$(date) $new_mac" >> $CONFIG_FILE
# }

# # Function to save the original MAC address to a write-only file
# save_original_mac() {
#     # Read the original MAC address from the history file
#     original_mac=$(tail -n 1 $CONFIG_FILE | awk '{print $8}')

#     # Save the original MAC address to the write-only file
#     echo "$original_mac"
#     echo "$original_mac" > $OG_MAC_FILE

#     # Display a confirmation message
#     echo "Original MAC address <$original_mac> saved to $OG_MAC_FILE (write-only)."
# }

# # Function to restore the original MAC address
# restore_original_mac() {
#     # Read the original MAC address from the write-only file
#     original_mac=$(cat $OG_MAC_FILE 2>/dev/null)

#     # Check if the write-only file exists and contains a MAC address
#     if [ -z "$original_mac" ]; then
#         echo "Original MAC address not found. Please save it first using option 4."
#         return
#     fi

#     # Disable the network interface
#     ifconfig $interface down

#     # Change the MAC address back to the original
#     ifconfig $interface hw ether $original_mac

#     # Enable the network interface
#     ifconfig $interface up

#     # Display the restored MAC address
#     echo "Restored original MAC address for $interface: $original_mac"
# }

# # Function to display MAC address history
# display_mac_history() {
#     echo "MAC address change history:"
#     cat $CONFIG_FILE
# }

# # Function to change the MAC address
# change_to_random_mac() {
#     # Generate a random MAC address
#     new_mac=$(printf '00:1D:%02X:%02X:%02X:%02X\n' $((RANDOM%256)) $((RANDOM%256)) $((RANDOM%256)) $((RANDOM%256)))

#     # Disable the network interface
#     ifconfig $interface down

#     # Change the MAC address
#     ifconfig $interface hw ether $new_mac

#     # Enable the network interface
#     ifconfig $interface up

#     # Display the new MAC address
#     echo "Changed MAC address of $interface to $new_mac"

#     # Save the new MAC address to the history file
#     echo "$(date) $new_mac" >> $CONFIG_FILE
# }

# # Check if the script is run as root
# if [[ $EUID -ne 0 ]]; then
#     echo "This script must be run as root."
#     exit 1
# fi

# # Check if the configuration file exists, create it if not
# if [ ! -e "$CONFIG_FILE" ]; then
#     touch "$CONFIG_FILE"
# fi

# # Check if the network interface is provided as an argument
# if [ -z "$1" ]; then
#     echo "Usage: $0 <interface>"
#     exit 1
# fi

# # Assign the provided interface to a variable
# interface=$1

# # Infinite loop
# while true; do
#     # Display the menu
#     echo "1. Display current MAC address"
#     echo "2. Change to random MAC address"
#     echo "3. Change to a specific MAC address"
#     echo "4. Save original MAC address to a write-only file"
#     echo "5. Restore original MAC address"
#     echo "6. Display MAC address change history"
#     echo "7. Exit"

#     # Read user choice
#     read -p "Enter your choice (1-7): " choice

#     # Perform the selected action
#     case $choice in
#         1) display_current_mac ;;
#         2) change_to_random_mac ;;
#         3) change_to_specific_mac ;;
#         4) save_original_mac ;;
#         5) restore_original_mac ;;
#         6) display_mac_history ;;
#         7) exit 0 ;;
#         *) echo "Invalid choice. Please try again." ;;
#     esac
# done


#!/bin/bash

# Configuration file to store MAC address history
CONFIG_FILE="$HOME/.macchanger_history"

# File to store the original MAC address in write-only mode
OG_MAC_FILE="$HOME/.original_mac_write_only"

# Function to display the current MAC address
display_current_mac() {
    current_mac=$(ifconfig $interface | awk '/ether/ {print $2}')
    echo "Current MAC address for $interface: $current_mac"
}

# Function to change the MAC address to a specific value
change_to_specific_mac() {
    # Read user input for the new MAC address
    new_mac=$(whiptail --inputbox "Enter the new MAC address:" 8 40 --title "Change MAC Address" 3>&1 1>&2 2>&3)
    exitstatus=$?
    if [ $exitstatus -ne 0 ]; then
        return
    fi

    # Validate the entered MAC address format
    if [[ ! $new_mac =~ "^([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}$" ]]; then
        whiptail --msgbox "Invalid MAC address format. Please enter a valid address." 8 40 --title "Error"
        return
    fi

    # Disable the network interface
    ifconfig $interface down

    # Change the MAC address
    ifconfig $interface hw ether $new_mac

    # Enable the network interface
    ifconfig $interface up

    # Display the new MAC address
    whiptail --msgbox "Changed MAC address of $interface to $new_mac" 8 40 --title "Success"

    # Save the new MAC address to the history file
    echo "$(date) $new_mac" >> $CONFIG_FILE
}

# Function to save the original MAC address to a write-only file
save_original_mac() {
    # Read the original MAC address from the history file
    original_mac=$(tail -n 1 $CONFIG_FILE | awk '{print $2}')

    # Save the original MAC address to the write-only file
    echo "$original_mac" > $OG_MAC_FILE

    # Display a confirmation message
    whiptail --msgbox "Original MAC address saved to $OG_MAC_FILE (write-only)." 8 40 --title "Success"
}

# Function to restore the original MAC address
restore_original_mac() {
    # Read the original MAC address from the write-only file
    original_mac=$(cat $OG_MAC_FILE 2>/dev/null)

    # Check if the write-only file exists and contains a MAC address
    if [ -z "$original_mac" ]; then
        whiptail --msgbox "Original MAC address not found. Please save it first using option 4." 8 40 --title "Error"
        return
    fi

    # Disable the network interface
    ifconfig $interface down

    # Change the MAC address back to the original
    ifconfig $interface hw ether $original_mac

    # Enable the network interface
    ifconfig $interface up

    # Display the restored MAC address
    whiptail --msgbox "Restored original MAC address for $interface: $original_mac" 8 40 --title "Success"
}

# Function to display MAC address history
display_mac_history() {
    whiptail --textbox --scrolltext $CONFIG_FILE 20 80 --title "MAC Address Change History"
}

# Check if the script is run as root
if [[ $EUID -ne 0 ]]; then
    whiptail --msgbox "This script must be run as root." 8 40 --title "Error"
    exit 1
fi

# Check if the configuration file exists, create it if not
if [ ! -e "$CONFIG_FILE" ]; then
    touch "$CONFIG_FILE"
fi

# Check if the network interface is provided as an argument
if [ -z "$1" ]; then
    whiptail --msgbox "Usage: $0 <interface>" 8 40 --title "Error"
    exit 1
fi

# Assign the provided interface to a variable
interface=$1

# Infinite loop
while true; do
    # Display the menu using whiptail
    choice=$(whiptail --title "MAC Address Changer" --menu "Choose an option:" 15 40 7 \
        "1" "Display current MAC address" \
        "2" "Change to random MAC address" \
        "3" "Change to a specific MAC address" \
        "4" "Save original MAC address to a write-only file" \
        "5" "Restore original MAC address" \
        "6" "Display MAC address change history" \
        "7" "Exit" 3>&1 1>&2 2>&3)

    # Check if user pressed Cancel or Esc
    if [ $? -ne 0 ]; then
        exit 0
    fi

    # Perform the selected action
    case $choice in
        1) display_current_mac ;;
        2) change_mac ;;
        3) change_to_specific_mac ;;
        4) save_original_mac ;;
        5) restore_original_mac ;;
        6) display_mac_history ;;
        7) exit 0 ;;
        *) whiptail --msgbox "Invalid choice. Please try again." 8 40 --title "Error" ;;
    esac
done
    